...
//TODO remove players that we haven't seen in awhile
...
function handlePlayer(player) {
    ...
    if (Math.abs(player.dx) > maxVelocity) { //slow down
        ...
        player.dx = (player.dx / Math.abs(player.dx)) * (Math.abs(player.dx) - friction);
        console.log(" 5 ")
    }
    ...
    if (Math.abs(player.dy) > maxVelocity) { //slow down
        ...
        player.dy = (player.dy / Math.abs(player.dy)) * (Math.abs(player.dy) - friction);
        console.log(" 5 ")
    }

    if (player.x + player.dx + gameState.options.block.width >= gameState.options.stage.width 
        || player.x + player.dx <= 0) {
        player.dx = -player.dx / 2;
    }
    if (player.y + player.dy + gameState.options.block.height >= gameState.options.stage.height || player.y + player.dy <= 0) {
...
io.on('connection', function (socket) {
    ...
    socket.on('disconnect', function (update) {
        ...
        if (player) {
            ...
            io.sockets.emit("player disconnected", player.userId);
